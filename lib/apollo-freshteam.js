...
async function fetchPage(path, page) {
  const pagePath = addToPath(path, {page});
  console.log(`Fetching page ${pagePath}`);
  const res = await freshClient.get(pagePath);
  return res.data;
}
...
async function loadAllFromPath(path) {
  ...
  if(res.headers["total-pages"]) {
    const totalPages = parseInt(res.headers["total-pages"]);
    const nextPages = await Promise.all(range(2, totalPages + 1).map(page => fetchPage(path, page)));
    return res.data.concat(...nextPages);
  }
  return res.data;
}

async function loadAll(path, args = {}) {
  return await loadAllFromPath(addToPath(path, args));
}
